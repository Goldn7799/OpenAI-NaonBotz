...
const os = require('os')
const { getQueue, removeQueue } = require('../OpenAI/Queue.js')
...
const start = () => {
  ...
  app.get('/data/:password', async (req, res) => {
    ...
    if (password === systemConf.interfaces.password) {
      data.os = await {
        arch: os.arch(),
        cpu: os.cpus(),
        freemem: os.freemem(),
  ...
  app.post('/request/setlimit/:limit/:password', async (req, res) => {
    ...
    if (password === systemConf.interfaces.password) {
      if (limit.length > 0 && parseFloat(limit)) {
        ...
        res.status(200).json({
          success: true,
          message: 'Succes Settng Limit'
        })
      } else {
        res.status(403).json({
          success: false,
          message: 'Failed Settng Limit'
        })
      }
    } else {
      res.status(403).json({
  ...
  app.post('/request/removequeue/:id/:password', async (req, res) => {
    const { password, id } = req.params
    if (password === systemConf.interfaces.password && id.length > 0) {
      if (await removeQueue(id)) {
        res.status(200).json({
          success: true,
          message: 'Succes Delete Queue'
        })
      } else {
        res.status(403).json({
          success: false,
          message: 'Failed Delete Queue'
        })
      }
    } else {
      res.status(403).json({
  ...
  app.listen(systemConf.interfaces.port, () => {
    console.log('\x1b[32m\x1b[1m%s\x1b[0m', '-- Interface Server Started --')
    console.log('Interface web port : \x1b[1m%s\x1b[0m', systemConf.interfaces.port)
  })
}
