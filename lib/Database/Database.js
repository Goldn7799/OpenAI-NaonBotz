...
const openaiLimitSet = (limit) => {
  database.state.openaiLimit = limit
  return true
}
/// / End Write
...
setInterval(() => {
  const date = new Date()
  if (dataStatsHistory.database.length > 20) {
    dataStatsHistory.database.shift()
    dataStatsHistory.database[0] = ['Time', 'Users', 'Groups']
  };
  if (dataStatsHistory.limit.length > 20) {
    dataStatsHistory.limit.shift()
    dataStatsHistory.limit[0] = ['Time', 'OpenAI Limit']
  };
  dataStatsHistory.database.push([`${utility.timeParse(date.getHours(), date.getMinutes())}`, (Object.keys(database.chats)).length, (Object.keys(database.groups)).length])
  dataStatsHistory.limit.push([`${utility.timeParse(date.getHours(), date.getMinutes())}`, (Number(database.state.openaiLimit)).toFixed(4)])
}, 300000)
              ...
              fs.readFile(`${pathto}/data-store/db/users.json`, 'utf-8', async (err) => {
                ...
                fs.readFile(`${pathto}/data-store/db/message.json`, 'utf-8', async (err) => {
                  ...
                  fs.readFile(`${pathto}/data-store/db/stats-history.json`, 'utf-8', async (err) => {
                    if (err) {
                      fs.writeFile(`${pathto}/data-store/db/stats-history.json`, JSON.stringify({database: [['Time', 'Users', 'Groups'], ['00:00', 0, 0]], limit:[['Time', 'OpenAI Limit'], ['00:00', 0]]}), (err) => {
                        if (err) {
