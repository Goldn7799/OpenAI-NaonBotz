...
setInterval(() => {
  ...
  if (dataStatsHistory.length > 20) {
    ...
    dataStatsHistory[0] = ['Time', 'Users', 'Groups', 'OpenAI Limit']
  };
  dataStatsHistory.push([`${utility.timeParse(date.getHours(), date.getMinutes())}`, (Object.keys(database.chats)).length, (Object.keys(database.groups)).length, (database.state.openaiLimit).toFixed(4)])
}, 300000)
              ...
              fs.readFile(`${pathto}/data-store/db/users.json`, 'utf-8', async (err) => {
                ...
                fs.readFile(`${pathto}/data-store/db/message.json`, 'utf-8', async (err) => {
                  ...
                  fs.readFile(`${pathto}/data-store/db/stats-history.json`, 'utf-8', async (err) => {
                    if (err) {
                      fs.writeFile(`${pathto}/data-store/db/stats-history.json`, JSON.stringify([['Time', 'Users', 'Groups', 'OpenAI Limit'], ['00:00', 0, 0, 0]]), (err) => {
                        if (err) {
