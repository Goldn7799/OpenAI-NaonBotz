...
primaryHost.on("qr", (qr)=>{
  ...
  interface.setState("Please Scan QR Code");
})
...
primaryHost.on("loading_screen", (state)=>{
  ...
  interface.setState(`Loading ${state}%`)
...
primaryHost.on("ready", ()=>{
  ...
  interface.setState("Client Ready");
})
...
primaryHost.on("authenticated", (session)=>{
  ...
  interface.setState("Connected To WhatsApp");
})
